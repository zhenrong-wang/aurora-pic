...
ParticleBoundary parse_particle_boundary(const KeyValue& kv, const std::string& key, ParticleBoundary def) {
    ...
    throw std::runtime_error("invalid 2D particle boundary value for '" + key + "': '" + value + "'");
}
...
Config load_config(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        if (line.front() == '[' && line.back() == ']') {
            ...
            if (section == "collisions") { current = &collision; allowed = &collision_keys; }
            else if (section == "species" || starts_with(section, "species.")) {
                species_blocks.emplace_back();
                current = &species_blocks.back();
                ...
                throw config_error(line_number, "unknown section '" + section + "'");
            }
            continue;
        }
        auto eq = line.find('=');
    ...
    cfg.species.clear();
    for (const auto& block : species_blocks) {
        SpeciesConfig s;
...
unsigned detect_config_dimension(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        if (dimension == 1 || dimension == 2) return static_cast<unsigned>(dimension);
        throw config_error(line_number, "dimension must be 1 or 2");
    }
    return 1;
}
...
Simulation2DConfig load_config_2d(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        if (line.front() == '[' && line.back() == ']') {
            section = lower(trim(line.substr(1, line.size() - 2)));
            if (section.empty()) throw config_error(line_number, "empty section name");
            if (section == "species" || starts_with(section, "species.")) {
                species_blocks.emplace_back();
                current = &species_blocks.back();
                allowed = &species_keys;
                if (starts_with(section, "species.")) {
                ...
                throw config_error(line_number, "unknown section '" + section + "' in 2D config");
            }
            continue;
        }
        auto eq = line.find('=');
