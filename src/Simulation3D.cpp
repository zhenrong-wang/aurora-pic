...
void Simulation3D::initialize() {
    ...
    if (cfg_.initial_state_path.empty()) {
            ...
            load_validated_external_particle_state_bounded(
                ...
                [&](std::size_t species_index,
                    ...
                    const double maximum_z =
                        species_config.init_z_max < 0.0
                            ? mesh_.length_z()
                            : species_config.init_z_max;
                    const bool outside =
                        record.position.x <
        ...
        if (initial_state_metadata_.velocity_staggering !=
            ...
            throw std::runtime_error(
                "3D simulation supports only time-centered external velocities");
        }
    }
    deposit_and_solve();
