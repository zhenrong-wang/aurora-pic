...
- `UnstructuredMesh2D`: computational state over validated imported topology, with lumped nodal control areas, spatially accelerated triangle/quad point location, conservative element-shape charge deposition, and nodal electric-field interpolation. It is a tested particle-grid coupling layer; the unstructured Poisson solve and runtime/config integration remain pending.
