...
- `solve_unstructured_poisson`: triangle/quad finite-element stiffness assembly, strict physical-label Dirichlet mapping, CSR storage, Jacobi-preconditioned conjugate gradients, convergence reporting, and nodal electric-field recovery.
- `UnstructuredSimulation2D`: imported-geometry runtime with area-uniform domain seeding, optional bounded initialization regions, electrostatic/Boris particle advance, earliest-crossing geometry-aware absorbing or reflecting boundary policies, per-label loss diagnostics, transient/steady execution, topology-checked restart, particle samples, and unstructured `.vtu` field output.
