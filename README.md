...
- `UnstructuredSimulation2D`: imported-geometry runtime with area-uniform domain seeding, optional bounded initialization regions, deterministic tagged-boundary particle injection, cached electrostatic field solves, electrostatic/Boris particle advance, earliest-crossing geometry-aware absorbing or reflecting boundary policies, per-label loss/source diagnostics, transient/steady execution, topology-checked restart, particle samples, and unstructured `.vtu` field output.
