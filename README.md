...
- `UnstructuredSimulation2D`: C++ runtime path with area-uniform domain seeding, optional bounded initialization regions, electrostatic/Boris particle advance, earliest-crossing geometry-aware absorbing or reflecting boundary policies, per-label loss diagnostics, transient/steady execution, and unstructured `.vtu` field output. Config/CLI and checkpoint integration remain pending.
...
This is a serious first version, not a final plasma platform. Key known gaps are: no MPI/GPU backend yet, OpenMP is limited to safe particle-loop slices rather than a whole-solver scaling model, simplified collision model, prescribed uniform magnetic fields only (no self-consistent electromagnetic field solve yet), and no config/CLI/checkpoint path around the current C++ imported-geometry simulation runtime. High-volume particle dumps are intentionally deferred to an openPMD/HDF5-style format in a later phase; current text checkpoint and particle CSV output are for restart, inspection, and regression/debug workflows. These extension points are documented in `docs/methodology.md`, `docs/multidimensional-roadmap.md`, and `docs/performance-envelope.md`.
