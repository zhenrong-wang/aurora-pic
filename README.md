...
This is a serious first version, not a final plasma platform. Key known gaps are: no MPI/GPU backend yet, OpenMP is limited to safe particle-loop slices rather than a whole-solver scaling model, simplified collision model, prescribed uniform magnetic fields only (no self-consistent electromagnetic field solve yet), imported field conditions are limited to label-wise constant Dirichlet/Neumann data, and the imported runtime has not been performance-qualified on large meshes. High-volume particle dumps are intentionally deferred to an openPMD/HDF5-style format in a later phase; current text checkpoint and particle CSV output are for restart, inspection, and regression/debug workflows.
