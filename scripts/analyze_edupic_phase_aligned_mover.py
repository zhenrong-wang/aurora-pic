#!/usr/bin/env python3
"""Analyze the prospective one-timestep RF phase-alignment control."""
...
def decision(ratios: list[float], gates_pass: bool) -> dict[str, bool]:
    ...
    return {
        ...
        "phase_timing_explains_deficit": gates_pass and parity,
        "partial_phase_effect": gates_pass and not (persistence or parity),
...
def analyze(rule_path: Path, crossing_paths: list[Path],
    ...
    baseline_ratios = [{
        promotion: float(member[promotion]) / float(
            baseline[state["id"]]["promotions"]),
        demotion: float(member[demotion]) / float(
            baseline[state["id"]]["demotions"]),
    ...
    linked = all(
        ...
        report.get("algorithm_contract", {}).get("phase_aligned_rad") ==
            rule["locked_inputs"]["phase_aligned_rad"]
    ...
    return {
        ...
        "scope": "one_timestep_rf_phase_alignment_result",
        "rule_sha256": rule_hash,
        "gates": gates,
        "all_runner_population_provenance_and_repeatability_gates_passed":
            all_gates,
        "critical_phase_0p125_to_0p5": {
            "phase_aligned_aurorapic_microstates": members,
            "native_edupic_ensemble_mean": native,
            "phase_aligned_aurorapic_to_native_ratios": ratios,
            "phase_aligned_aurorapic_to_pooled_baseline_ratios":
                baseline_ratios,
            "phase_aligned_aurorapic_relative_range": ranges,
        },
