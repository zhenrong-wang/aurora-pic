...
Post-M6 production hardening has unified transient and steady-state run semantics across 1D, 2D, and 3D. Every structured simulation now reports whether the energy-window convergence criterion was reached, remains bounded by a configured step cap, and writes a final checkpoint when steady termination occurs. Imported 2D topology is validated as a finite, nondegenerate, consistently oriented, manifold domain with exact tagged-boundary closure. `UnstructuredMesh2D` now adds indexed nodal state, lumped control areas, spatially accelerated triangle/quad point location, conservative element-shape charge deposition, and nodal field interpolation. The next geometry milestone is boundary-condition mapping and sparse unstructured Poisson assembly/solution, followed by runtime/config integration.
