...
| M3 | Current baseline | Scalable data and restart formats | Added VTK XML structured-grid field output with legacy/XML/both format selection and compatibility tests as the first scalable data-format step; later openPMD/HDF5 work remains planned. |
| M4 | Next | Runtime scaling backend | Introduce OpenMP/MPI/GPU-ready interfaces only after validation remains stable; include deterministic single-rank comparisons and scaling smoke tests. |
