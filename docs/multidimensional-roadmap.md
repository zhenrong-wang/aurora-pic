...
Post-M6 production hardening has unified transient and steady-state run semantics across 1D, 2D, and 3D. Every structured simulation now reports whether the energy-window convergence criterion was reached, remains bounded by a configured step cap, and writes a final checkpoint when steady termination occurs. Imported 2D topology is now validated as a finite, nondegenerate, consistently oriented, manifold domain with exact tagged-boundary closure, and exposes the cell and face metrics needed by later discretizations. The next geometry milestone is to make that topology executable with shape-function deposition/interpolation, boundary-condition mapping, and an unstructured Poisson solve.
