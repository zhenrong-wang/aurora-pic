...
| M2 | Complete | Geometry and mesh import workflow | Added tagged 2D Gmsh v2 ASCII import into internal boundary labels without exposing solver code to file-format details; documented CAD/surface-to-mesh preprocessing. |
...
Post-M6 production hardening has unified transient and steady-state run semantics across 1D, 2D, and 3D. Every structured simulation now reports whether the energy-window convergence criterion was reached, remains bounded by a configured step cap, and writes a final checkpoint when steady termination occurs. The next geometry milestone is to turn the existing tagged `ImportedMesh2D` topology into a validated executable mesh path with shape-function deposition/interpolation, boundary-condition mapping, and an unstructured Poisson solve.
