...
Post-M6 production hardening has unified transient and steady-state run semantics across 1D, 2D, and 3D. Every structured simulation now reports whether the energy-window convergence criterion was reached, remains bounded by a configured step cap, and writes a final checkpoint when steady termination occurs. Imported 2D topology is validated as a finite, nondegenerate, consistently oriented, manifold domain with exact tagged-boundary closure. `UnstructuredMesh2D` adds indexed nodal state, lumped control areas, spatially accelerated triangle/quad point location, conservative element-shape charge deposition, and nodal field interpolation. The imported field path assembles triangle/quad finite-element stiffness terms, maps physical labels to strict Dirichlet values, solves the CSR system with Jacobi-preconditioned conjugate gradients, reports convergence, and recovers nodal electric fields. `UnstructuredSimulation2D` now closes the C++ transient/steady loop with area-uniform initialization, segment-based absorbing/reflecting boundary interactions, per-label diagnostics, and `.vtu` output. The next geometry milestone is config/CLI selection plus checkpoint/restart and particle-output integration.
