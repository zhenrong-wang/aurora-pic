...
For 1D species subcycling, `subcycle_charge_deposition = current_position`
is the production default and deposits from the latest particle positions.
The explicit `pre_push_held` control instead caches a subcycled species'
nodal charge before constructing the field for each upcoming due push and
retains that density until the next due field time. Checkpoint v24 stores
these caches exactly and rejects policy mismatches.
