...
| `examples/hall_field_profile_smoke.cfg` | 2D3V | 16 x 8 nodes | 8 initial / 16 final | 4 | Single-thread SI smoke for a magnetic profile, 200 V axial drop, mixed topology, eV-resolved initial/source Maxwellians, peak-volumetric pair injection, charge-regulated cathode source, and emission-plane potential reference; not a HET discharge result. |
...
| `examples/imported_charge_exchange_2d.cfg` | 2D3V | 6 nodes / 3 mixed cells | 64 | 6 | External synthetic `.gas` manifest, finite-mass resonant ion-neutral charge exchange, energy transfer to the neutral reservoir, and checkpoint v6. |
