...
| `examples/hall_field_profile_smoke.cfg` | 2D3V | 16 x 8 nodes | 8 initial / 16 final | 4 | Single-thread SI smoke for a magnetic profile, 200 V axial drop, mixed topology, explicit extrusion depth, peak-volumetric pair injection, charge-regulated cathode source, and emission-plane potential reference; not a HET discharge result. |
